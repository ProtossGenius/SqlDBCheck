...
function doCheck(lineData, db) {
     ...
     */
    db.checkStringVal("user", "id", uid);//When there is only one primary key
    db.checkValExist("items", "`id`='" + id + "'");//When there are multiple primary keys or need a special query
}
